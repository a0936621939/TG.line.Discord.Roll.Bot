...
coc7 成功成長或增長檢定： .dp 或 成長檢定 或 幕間成長 (技能%) (名稱) \n\
例）.DP 50 騎馬 | 成長檢定 45 頭槌 | 幕間成長 40 單車\n\
coc7 大成功成長或增長檢定： .dpb 或 成長檢定大 或 幕間成長大 (技能%) (名稱) \n\
例）.DPB 50 騎馬 | 成長檢定大 45 頭槌 | 幕間成長大 40 單車\n\
coc7 幸運成長檢定： .dpl 或 成長檢定運 或 幕間成長運 (技能%) (LUC) \n\
例）.DPL 50 LUC | 成長檢定運 45 LUC | 幕間成長運 40 LUC\n\
...
var rollDiceCommand = async function ({
	...
	if ((trigger == '.dp' || trigger == '成長檢定' || trigger == '幕間成長') && mainMsg[1] <= 1000) {
		rply.text = await DevelopmentPhasenormal(mainMsg[1], mainMsg[2]);
	}
	if ((trigger == '.dpb' || trigger == '成長檢定大' || trigger == '幕間成長大') && mainMsg[1] <= 1000) {
		rply.text = await DevelopmentPhasebig(mainMsg[1], mainMsg[2]);
	}
	if ((trigger == '.dpl' || trigger == '成長檢定運' || trigger == '幕間成長運') && mainMsg[1] <= 1000) {
		rply.text = await DevelopmentPhaseLUC(mainMsg[1], mainMsg[2]);
	}
...
async function DevelopmentPhasenormal(target, text) {
	let result = '';
	if (text == undefined) text = "";
	let skill = await rollbase.Dice(100);
	let improved = await rollbase.Dice(5);
	if (target > 95) target = 95;
	if (skill == 1) improved = 1;
	if (skill <= 5 && target >= 11) improved = 1;
	if (skill == 100) improved = 5;
	if (skill >= 96 && target <= 89) improved = 5;
	if (skill > target) result = "成長或增強檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 成功!\n你的技能增加1D5(" + improved + ")點!";
	if (skill <= target) result = "成長或增強檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 失敗!\n你的技能沒有變化!";
	if (skill == 1) result = "成長或增強檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 大失敗!\n你的技能增加" + improved + "點!";
	if (skill <= 5 && target >= 11) result = "成長或增強檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 大失敗!\n你的技能增加" + improved + "點!";
	if (skill == 100) result = "成長或增強檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 大成功!\n你的技能增加" + improved + "點!";
	if (skill >= 96 && target <= 89) result = "成長或增強檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 大成功!\n你的技能增加" + improved + "點!";
	return result;
}

async function DevelopmentPhasebig(target, text) {
	let result = '';
	if (text == undefined) text = "";
	let skill = await rollbase.Dice(100);
	let improved = await rollbase.Dice(5);
	if (target > 95) target = 95;
	if (skill > target) improved += 5;
	if (skill == 1) improved = 5;
	if (skill <= 5 && target >= 11) improved = 5;
	if (skill == 100) improved = 10;
	if (skill >= 96 && target <= 89) improved = 10;
	if (skill > target) result = "成長或增強檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 成功!\n你的技能增加1D5+5(" + improved + ")點!";
	if (skill <= target) result = "成長或增強檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 失敗!\n你的技能增加1D5(" + improved + ")點!";
	if (skill == 1) result = "成長或增強檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 大失敗!\n你的技能增加" + improved + "點!";
	if (skill <= 5 && target >= 11) result = "成長或增強檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 大失敗!\n你的技能增加" + improved + "點!";
	if (skill == 100) result = "成長或增強檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 大成功!\n你的技能增加" + improved + "點!";
...
async function DevelopmentPhaseLUC(target, text) {
	let result = '';
	if (text == undefined) text = "";
	let skill = await rollbase.Dice(100);
	let improved = await rollbase.Dice(5);
	...
	if (skill == 100) improved += 10;
	if (skill >= 96 && target <= 89) improved += 10;
	if (skill > target) result = "幸運成長檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 成功!\n你的幸運增加1D5+5(" + improved + ")點!";
	if (skill <= target) result = "幸運成長檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 失敗!\n你的幸運增加1D5(" + improved + ")點!";
	if (skill == 1) result = "幸運成長檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 大失敗!\n你的幸運沒有變化!";
	if (skill <= 5 && target >= 11) result = "幸運成長檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 大失敗!\n你的幸運沒有變化!";
	if (skill == 100) result = "幸運成長檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 大成功!\n你的幸運增加1D5+10(" + improved + ")點!";
	if (skill >= 96 && target <= 89) result = "幸運成長檢定: " + text + "\n1D100 ≦ " + target + "\n" + skill + " → 大成功!\n你的幸運增加1D5+10(" + improved + ")點!";
	return result;
}
...
async function coc7(chack, text) {
	...
	if (temp <= 5 && chack >= 11) result = '1D100 ≦ ' + chack + "：\n" + temp + ' → 恭喜！大成功！';
	if (temp == 100) result = '1D100 ≦ ' + chack + "：\n" + temp + ' → 啊！大失敗！';
	if (temp >= 96 && chack <= 89) result = '1D100 ≦ ' + chack + "：\n" + temp + ' → 啊！大失敗！';
...
async function coc7chack(temp, chack, text) {
	if (text == null) {
		...
		if (temp <= 5 && chack >= 11) return temp + ' → 恭喜！大成功！';
		if (temp == 100) return temp + ' → 啊！大失敗！';
		if (temp >= 96 && chack <= 89) return temp + ' → 啊！大失敗！';
		...
		if (temp <= 5 && chack >= 11) return temp + ' → 恭喜！大成功！' + text;
		if (temp == 100) return temp + ' → 啊！大失敗！：' + text;
		if (temp >= 96 && chack <= 89) return temp + ' → 啊！大失敗！：' + text;
